...
const Information = () => {
    ...
    const getUserInfo = async () => {
        const res = await fetch(process.env.REACT_APP_API +`/users/me`,{
          ...
          if(json.success) setUserInfo(json.data)
        ...
        <div className='info'>
            ...
            <form>
                ...
                <div  className="form-field mb-3 user-avatar">
                    ...
                    <img src={userInfo.avatar} className="d-block mx-auto rounded-circle w-100"/>
                    </label>
                </div>
                <div>
                <label htmlFor='name' className=' mb-2'>Name<h6 className='star'>*</h6></label>
                        <input type='text' id='name' className='form-control mb-3' value={userInfo.name} />

                        <label htmlFor='email' className='mb-2'>Ema il Address<h6 className='star'>*</h6></label>
                        <input value ={userInfo.email}  type='email' id='email' className='form-control mb-3' />
            ...
            <ul className="list-group">
                {userInfo?.posts?.map((item,i)=>(

                     <li className='list-group-item d-flex align-items-center justify-content-between'>
                     <span>{item.content}</span>
                     <span><button class="btn btn-danger btn-sm">Delete</button></span>
                     </li>
